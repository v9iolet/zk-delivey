	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('fuwuxiangmu','下单')" @click="dingdanxiadanonAcross('下单','','','','')" type="warning">下单</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('fuwuxiangmu','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('fuwuxiangmu','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const dingdanxiadanonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('fuwuxiangmu',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/dingdanxiadanAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
