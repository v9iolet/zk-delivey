<template>
	<div class="home">
		<div class="index_top">
			<div class="index_top_title"><span>基于springboot的湛科校园包裹代取服务平台</span></div>
			<div class="index_top_right">
				<el-button v-if="!Token" class="login" type="success" icon="User" circle @click="loginClick">
					登录
				</el-button>
				<el-button v-if="Token" type="danger" icon="SwitchButton" circle @click="loginOut" class="loginOut">
					退出
				</el-button>
			</div>
		</div>
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'kuaidiyuan'){
				context?.$toolUtil.storageSet('headportrait',res.data.data.touxiang||'')
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'kuaidiyuan'){
				context?.$toolUtil.storageSet("frontName", res.data.data.kuaidiyuanzhanghao)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'wuliugongsi'){
...
.menu-wrapper .first-item.is-active {
    ...
    box-shadow: 0px 0 0px 0px #BA6A5D, 0px 0 0px 0px #BA6A5D, inset 0px 1px 7px 0px #666;
}
...
li.el-menu-item.second-item:hover,li.el-menu-item.second-item.is-activ {
    color: var(--theme)!important;
}
